...
#define L_S A1      // IR sensor Right
#define R_S 3      // IR sensor Left
...
analogWrite(enB, 85); // Write The Duty Cycle 0 to 255 Enable Pin B for Motor2 Speed 
    ...
    if (distance_F > Set) {
        if (digitalRead(middle_ir_left) == HIGH) {
            ...
            if (digitalRead(middle_ir_left) == HIGH) {
                ...
                for (int k = 80; k <= 90; k++) {  // top arm motor start
                    servoPulse(servo3, k);
                    delay(5);
                }

                int x = analogRead(moistureSensorPin);
                Serial.print(x);
                
                if( x > 800) 
                {
                  digitalWrite(waterpump,LOW);                 
                  Serial.println("No water detected. Digital output deactivated.");
                  delay(600);
                  digitalWrite(waterpump,HIGH);
                  servoPulse(servo, 70);
                }
                ...
                {
                  digitalWrite(waterpump,HIGH);
                  Serial.println("Water detected! Digital output activated.");    
                  servoPulse(servo, 70);            
                }
                delay(3000);
                
                for (int l = 90; l >= 80; l--) {  // top arm motor retract
                    servoPulse(servo3, l);
                    delay(10);
                }
            ...
            if (digitalRead(middle_ir_right) == HIGH) {
              // Perform the desired function
                Stop();
                for (int angle = 70; angle <= 160; angle++) {
                    servoPulse(servo1, angle);
                    delay(delayTime);
                }
                ...
                // Move from end position back to start position
                for (int angle = 160; angle >= 70; angle--) {
                    servoPulse(servo1, angle);
                    delay(delayTime);
                }
